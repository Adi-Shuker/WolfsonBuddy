...
const getSurvey = (req, res) => {
  ...
  executeQuery(
    "select q.id as question_id, q.text as question_text,qt.type as question_type, group_concat(qsa.suggested_answer) as suggested_answers " +
      "from surveys as s join questions as q on s.department_id=" +
      department_id +
...
const addQuestion = (req, res) => {
  ...
  executeQuery(query)
    ...
    .then(() => {
      query =
        "insert into questions(type_id, survey_id,text) values(" +
        question_type_id +
        "," +
        survey_id +
        ", '" +
      ...
      executeQuery(query)
        .then((data) => {
          ...
          if (suggested_answers !== null && suggested_answers.length > 0) {
            query +=
              "insert into question_suggested_answers(question_id, suggested_answer) values ";
            suggested_answers.forEach((element, index) => {
              suggested_answers[index] =
                "( LAST_INSERT_ID(), " + "'" + element + "') ";
            });
...
const deleteQuestion = (req, res) => {
  ...
  var query =
    "delete from question_suggested_answers as qsa where qsa.question_id=" +
    question_id +
    ";";
  executeQuery(query).then((data) => {
    query =
      "delete from users_answers as ua where ua.question_id=" +
      question_id +
      ";";
    executeQuery(query)
      .then((resData) => {
...
const getSurveyResult = (req, res) => {
  ...
  executeQuery(
    "select ua.question_id, group_concat(ua.answer) as answers,qt.type as question_type, q.text as " +
      "question_text, group_concat(qsa.suggested_answer) as suggested_answers from surveys as s " +
      "join questions as q on s.department_id=" +
      department_id +
...
const addStaffMember = (req, res) => {
  ...
  if (
    ...
    file.mv("./images/teamMembersImages/" + img_name, function (err) {
      ...
      var query =
        "insert into staff(name, department_id, role, description, picture, phone_number, clinical_practice,scientific_practice, academic_experience, professional_unions,education)" +
        " values ('" +
        name +
        "', '" +
        department_id +
...
const getStaffMembers = (req, res) => {
  console.log("in getStaffMembers");
  executeQuery(
    "select s.name as member_name, d.name as department_name, s.role,description, s.picture as picture," +
...
const addUpdate = (req, res) => {
  ...
  const query =
    "insert into announcements_and_updates(title, post_date, content, link) values('" +
    title +
    "','" +
    date +
    "','" +
...
const deleteNews = (req, res) => {
  ...
  const query =
    "delete from announcements_and_updates where id=" + news_id + ";";
  executeQuery(query)
    .then((resData) => {
      res.status(200).json(resData);
    })
